        ...
        if entry.file_type().is_dir() {
            ...
            if let Ok(entries) = fs::read_dir(path) {
                for subdir_entry in entries.filter_map(Result::ok) {
                    ...
                    if subdir_path.is_dir() && !exclude_patterns.iter().any(|p| subdir_path.to_string_lossy().contains(p)) {
...
fn process_file_info(filepath: &Path) -> Result<FileInfo> {
    ...
    let tokens = if binary { 0 } else { (size as f64 * TOKENS_PER_BYTE).ceil() as usize };
...
pub fn show_dir_info(dir_path: &Path, info: &DirInfo) {
    ...
    println!("{}", "=".repeat(60));
    println!("Files: {} ({} text)", info.total_files, info.total_files - info.binary_files);
