...
use crate::file_analysis::{FileInfo, DirectoryMap, is_binary, CLAUDE_TOKEN_LIMIT};
use crate::context_files::{ContextFile, get_or_rotate_file, append_to_file};
...
pub fn process_files(
    ...
    let filtered_files: Vec<_> = files.iter().filter(|f| !f.binary && f.tokens <= max_tokens).collect();
    for file in &filtered_files {
        let filepath = &file.path;
        let rel_path = filepath.strip_prefix(std::env::current_dir()?).unwrap_or(filepath);
        ...
        if summarize {
            ...
            if current_context_file.current_tokens + (file.tokens / 4) > CLAUDE_TOKEN_LIMIT {
                current_context_file = get_or_rotate_file(&current_context_file, total_files, base_dir, output_dir)?;
...
pub fn process_directory_content(
    ...
    let rel_path = dir_path.strip_prefix(std::env::current_dir()?).unwrap_or(dir_path);
    let mut content = format!("\n\n## DIRECTORY: {}\n", rel_path.display());
    
    if action == &Action::Stats {
        if let Some(info) = dir_info.get(dir_path) {
            content.push_str(&format!("Files: {} ({} text)\n", 
                info.total_files, 
    ...
    match action {
        Action::Read => {
            ...
            if context_file.current_tokens + tokens > CLAUDE_TOKEN_LIMIT {
                *context_file = get_or_rotate_file(context_file, total_files, base_dir, output_dir)?;
        ...
        Action::Summarize => {
            ...
            // Run Claude if available (placeholder)
            // We'd run the actual claude command here, but for now just add a placeholder
            let summary = "Summary would be generated by claude if available.\n";
        ...
        Action::Stats => {
            ...
            let content = format!(
                "\n\n# File: {}\nSize: {} bytes\nEstimated tokens: {}\nExtension: .{}\n",
                rel_path.display(), size, tokens, ext
            );
...
pub fn process_node(
    ...
    if path.is_file() {
        ...
        match action {
            ...
            Action::Exclude => {
                info!("Excluding: {}", path.strip_prefix(std::env::current_dir()?).unwrap_or(path).display());
                processed.insert(path.to_path_buf());
            },
            _ => {}
        }
                ...
                if let Some(info) = dir_info.get(path) {
                    for file in &info.files {
                        if !file.binary && !processed.contains(&file.path) {
                            let (new_total, new_processed, new_included, new_used_files) = process_node(
                                &file.path,
                                dir_info,
                                context_file,
                                max_tokens,
    ...
    Ok((total_tokens, processed, included_files, unique_context_files))
}
...
pub fn apply_cached_actions(
    ...
    for path in paths {
        if !path.exists() {
            info!("Skipping cached path that no longer exists: {}", path.display());
            continue;
        }
        
        if let Some(action_str) = cache.get(path) {
            if let Some(action) = Action::from_str(action_str) {
                info!("Applying cached action '{}' to {}", action_str, path.display());
