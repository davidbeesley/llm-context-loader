...
use crate::file_analysis::{analyze_directory, is_binary, show_dir_info, CLAUDE_TOKEN_LIMIT};
use crate::cache::{load_cache, save_cache, get_action_for_path, should_prompt_for_directory};
use crate::processing::{process_node, apply_cached_actions, Action};
use crate::context_files::{ContextFile, create_context_file, finalize_context_files, get_or_rotate_file, append_to_file};
...
fn main() -> Result<()> {
    ...
    let start_dir = PathBuf::from(&args.start_dir).canonicalize()
    ...
    let total_tokens: usize = dir_info.values()
        .map(|dir| dir.tokens)
        .sum();
        
    let estimated_files = std::cmp::max(1, (total_tokens as f64 / CLAUDE_TOKEN_LIMIT as f64).ceil() as usize);
    
    info!("Estimated total tokens: {}", total_tokens);
    info!("Estimated context files needed: {}", estimated_files);
    
    // Create the first output file
    let mut context_file = create_context_file(1, estimated_files, &start_dir, args.output_dir.as_deref())?;
    ...
    if use_cache {
        ...
        for (path, action) in &cache {
            action_paths.entry(action.clone()).or_default().push(path.clone());
        }
        
        println!("\nCache summary:");
        for (action, paths) in action_paths.iter() {
            println!("\n  {}: {} items", action, paths.len());
            for path in paths.iter().take(5) { // Only show first 5 for brevity
                println!("    - {}", path.strip_prefix(&start_dir).unwrap_or(path).display());
            }
            if paths.len() > 5 {
        ...
        if use_cache {
            ...
            let (total_tokens, processed, included_files, mut used_files) = apply_cached_actions(
                &dir_info,
                &mut context_file,
                args.max_tokens,
                &cache,
    ...
    if claude_available && !all_context_files.is_empty() {
        ...
        if response.trim().to_lowercase() != "n" {
            ...
            let message = format!("The context file is at {}. Read that file in its entirety, then say 'Ready'.", 
...
fn process_interactive_loop(
    ...
    let result: Result<()> = (|| {
        while let Some(current) = to_process.pop() {
            ...
            if !is_file && use_cache && cached_action.as_deref() == Some("enter") {
                println!("\nAutomatically entering directory (from cache): {}", current.display());
                
                // Add a header for the directory - check token limit first
                if context_file.current_tokens + 200 > CLAUDE_TOKEN_LIMIT { // Rough estimate
                    *context_file = get_or_rotate_file(context_file, total_files, &start_dir, output_dir)?;
            ...
            if !is_file && use_cache && !should_prompt_for_directory(&current, &dir_info, cache) {
                println!("\nUsing cached actions for directory: {}", current.display());
            ...
            match choice.as_str() {
                "1" => { // Read
                    ...
                    let (new_total, new_processed, new_included, mut new_context_files) = process_node(
                        &current,
                        &dir_info,
                        context_file,
                        max_tokens,
                    ...
                    for file in new_context_files.into_iter().skip(1) { // Skip first as it's the updated original
    ...
    // Display information
    println!("\nProcessed {} nodes, {} files included.", processed.len(), included_files.len());
    println!("Estimated tokens: {}", total_tokens);
    println!("Created {} context files (limited to ~{} tokens each):", all_context_files.len(), CLAUDE_TOKEN_LIMIT);
